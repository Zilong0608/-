...
// English display labels for question bank categories.
// NOTE: option `value` must stay as the original DB category key (Chinese)
// so backend matching keeps working — only the label is translated.
const CATEGORY_LABELS_EN: Record<string, string> = {
  "LLM": "LLM / Large Models",
  "LLM大模型": "LLM / Large Models",
  "Oracle": "Oracle Database",
  "Oracle数据库": "Oracle Database",
  "SLAM": "SLAM / Computer Vision",
  "SLAM定位/视觉": "SLAM / Computer Vision",
  "Web3": "Web3 / Blockchain",
  "Web3区块链": "Web3 / Blockchain",
  "数据库": "Database Fundamentals",
  "数据库基础": "Database Fundamentals",
  "测试": "Software Testing",
  "测试开发": "Software Testing",
  "算法": "Algorithms & Graphics",
  "算法与图形": "Algorithms & Graphics",
  "网安": "Network Security",
...
const FALLBACK_CATEGORY_OPTIONS = [
  { label: "LLM / Large Models", value: "LLM" },
  { label: "Oracle Database", value: "Oracle" },
  { label: "SLAM / Computer Vision", value: "SLAM" },
  { label: "Web3 / Blockchain", value: "Web3" },
  { label: "Database Fundamentals", value: "数据库" },
  { label: "Software Testing", value: "测试" },
  { label: "Algorithms & Graphics", value: "算法" },
  { label: "Network Security", value: "网安" },
  { label: "General", value: "其他" }
];
...
export default function App() {
  ...
  const radarData = report
    ? [
        { subject: "Technical Accuracy", A: Math.round((report.avg_technical_accuracy || 0) * 10), fullMark: 100 },
        { subject: "Clarity", A: Math.round((report.avg_clarity || 0) * 10), fullMark: 100 },
        { subject: "Depth & Breadth", A: Math.round((report.avg_depth_breadth || 0) * 10), fullMark: 100 },
        { subject: "Correct Rate", A: Math.round((report.correct_rate || 0) * 100), fullMark: 100 }
      ]
    : [
        { subject: "Technical Accuracy", A: 0, fullMark: 100 },
        { subject: "Clarity", A: 0, fullMark: 100 },
        { subject: "Depth & Breadth", A: 0, fullMark: 100 },
        { subject: "Correct Rate", A: 0, fullMark: 100 }
      ];

  const currentCategoryLabel =
    categoryOptions.find(opt => opt.value === config.bank)?.label || "All Question Banks";

  const overallScoreDisplay = report ? Math.round(report.overall_score * 10) : 0;
  const strengthText = report?.strong_areas?.length ? report.strong_areas.join("; ") : "None yet";
  const weaknessText = report?.weak_areas?.length ? report.weak_areas.join("; ") : "None yet";
  ...
  async function loadCategories() {
    try {
      ...
      const rawOptions = data.map((item: any) => {
        ...
        return {
          label: CATEGORY_LABELS_EN[name] || CATEGORY_LABELS_EN[item.key] || name,
          value: item.key
        };
      });
      const seen = new Set<string>();
      const options = [ALL_BANKS_OPTION];
      ...
      // If the backend returned no categories, keep the fallback list
      // so the selector never disappears from the setup page.
      if (options.length === 1) {
        options.push(...FALLBACK_CATEGORY_OPTIONS);
      }
      setCategoryOptions(options);
    } catch (err) {
      setStatusMessage("Failed to load question categories");
      setCategoryOptions([ALL_BANKS_OPTION, ...FALLBACK_CATEGORY_OPTIONS]);
    }
  }
  ...
  async function loadPersonalities() {
    try {
      ...
      setStatusMessage("Failed to load interviewer personas");
    }
  }
  ...
  async function startInterview() {
    ...
    try {
      ...
      setStatusMessage("Failed to start the interview — please check that the backend is running");
    } finally {
  ...
  async function submitAnswer() {
    ...
    if (!answer) {
      setStatusMessage("Please type an answer first");
      return;
    }
    ...
    try {
      ...
      setStatusMessage("Failed to submit answer — please retry");
    } finally {
  ...
  async function fetchNextQuestion() {
    ...
    try {
      ...
      setStatusMessage("Failed to fetch the next question");
    }
  }
  ...
  async function endInterview() {
    ...
    try {
      ...
      setStatusMessage("Failed to generate the report");
    } finally {
           ...
           <main className="flex-1 flex flex-col items-center justify-between relative w-full max-w-4xl mx-auto px-6 overflow-hidden min-h-0">
                    ...
                    {isTextInputOpen ? (
                             ...
                             <div className="relative z-10">
                                ...
                                <div className="relative group/input">
                                   <textarea 
                                      ...
                                      placeholder="Write your thoughts on this crystal... (English is fine — it will be matched automatically)"
                                      value={textAnswer}
                       ...
                       >
                          ...
                          <div className={`absolute -bottom-16 left-1/2 -translate-x-1/2 whitespace-nowrap transition-all duration-500 ${isRecording ? 'opacity-0 translate-y-2' : 'opacity-100 translate-y-0'}`}>
                             <span className="text-slate-400 text-sm font-medium tracking-wide">Tap the orb to start answering</span>
                          </div>
                       </motion.div>
                    )}
                 </AnimatePresence>
              ...
              <div className="w-full pb-8 md:pb-10 grid grid-cols-1 md:grid-cols-3 gap-6 items-center shrink-0">
                 ...
                 <div className="flex justify-center md:justify-end">
                    ...
                    >
                       {questionIndex < MAX_QUESTIONS - 1 ? 'Skip / Next' : 'Finish Interview'}
            ...
            <main className="flex-1 w-full max-w-6xl mx-auto px-6 pb-12 grid grid-cols-1 md:grid-cols-12 gap-6 overflow-hidden min-h-0">
                {!showReportContent && (
                  <div className="md:col-span-12 bg-white/60 backdrop-blur-xl border border-white/60 p-10 rounded-[2rem] shadow-sm text-center">
                     <div className="text-slate-500 font-medium mb-3">Generating your report...</div>
                       ...
                       <div className="space-y-3">
                          ...
                          <div className="space-y-4">
                             {detailItems.length ? (
                               detailItems.map((item, index) => (
                                 ...
                                 >
                                   ...
                                   <div className="mt-3 space-y-2 text-sm">
                                      <div>
                                         <div className="text-xs uppercase tracking-widest text-slate-400">Question</div>
                                         <div className="text-slate-700">{item.question || "N/A"}</div>
                                      </div>
                                      <div>
                                         <div className="text-xs uppercase tracking-widest text-slate-400">Your Answer</div>
                                      ...
                                      {item.weaknesses?.length ? (
                                        <div>
                                           <div className="text-xs uppercase tracking-widest text-slate-400">Weaknesses</div>
                                           <div className="text-slate-600">{item.weaknesses.join("; ")}</div>
                                        </div>
                                      ) : null}
                                      {item.suggestions?.length ? (
                                        <div>
                                           <div className="text-xs uppercase tracking-widest text-slate-400">Suggestions</div>
                               ...
                               <div className="text-sm text-slate-400">No details yet</div>
                             )}
                          </div>
                       </div>
                    </div>

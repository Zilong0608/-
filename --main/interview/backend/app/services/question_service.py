...
class QuestionRepository:
    ...
    def list_categories(self) -> List[Dict[str, Any]]:
        ...
        向量库没有单独的分类表，这里优先从向量库 metadata 中统计
        category 字段；若拿不到，则回退到固定分类表，保证前端
        开始页面的题型分类选择不会消失。
        """
        counts: Dict[str, int] = {}
        try:
            store = getattr(self.vector_store, "_store", None)
            collection = getattr(store, "collection", None) or getattr(store, "_collection", None)
            if collection is not None:
                results = collection.get(include=["metadatas"])
                for metadata in results.get("metadatas") or []:
                    key = (metadata or {}).get("category") or (metadata or {}).get("category_key")
                    if key:
                        counts[key] = counts.get(key, 0) + 1
        except Exception as e:
            logger.warning(f"Failed to collect categories from vector store: {e}")

        if counts:
            return [
                {
                    "key": key,
                    "name": self._category_display_map.get(key, key),
                    "count": count
                }
                for key, count in sorted(counts.items())
            ]

        # 回退：固定分类表（count 未知记 0）
        return [
            {"key": key, "name": name, "count": 0}
            for key, name in self._category_display_map.items()
        ]

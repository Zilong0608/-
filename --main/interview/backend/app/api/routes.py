    ...
    try:
        ...
        if queued:
            background_tasks.add_task(
                ...
                session_id,
                result["question"],
                result.get("answer", request.answer),
                result["personality"],
                result["job_type"]
            )
...
async def speech_to_text(
    ...
    try:
        ...
        # 不再强制中文：不传 language 时自动检测，支持英文回答
...
async def realtime_stt(websocket: WebSocket):
    ...
    # 不再默认中文：未指定语言时自动检测，支持英文回答
    language = (websocket.query_params.get("language") or "").strip() or None
            ...
            session_update = {
                ...
                "session": {
                    ...
                    "input_audio_transcription": (
                        ...
                        if language else {"model": "whisper-1"}
                    ),
                    "turn_detection": {
                        "type": "server_vad",
                        "threshold": 0.3,

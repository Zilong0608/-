...
class InterviewEngine:
    ...
    def _is_mostly_english(text: str) -> bool:
        ...
        if letters < 3:
            return False
        # 中文字符占比极低时视为英文回答
        return cjk == 0 or cjk < letters * 0.05

    def _normalize_answer(self, answer: str) -> str:
        """
        若回答为英文，先翻译成中文再入库/评估，保证与中文题库匹配。
        翻译失败时使用原文，不阻断流程。
        """
        if not answer:
            return answer
        if not self._is_mostly_english(answer):
            return answer
        try:
            translated = self.ai_service.translate_to_chinese(answer)
            if translated and translated.strip():
                logger.info("English answer translated to Chinese for matching")
                return translated
        except Exception as e:
            logger.warning(f"Answer translation failed, using original text: {e}")
        return answer
